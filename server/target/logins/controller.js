...
let LoginController = class LoginController {
    async authenticate({ email, password }) {
        const teacher = await entity_1.Teacher.findOne({ where: { email } });
        if (!teacher)
            throw new routing_controllers_1.BadRequestError('A teacher with this email does not exist');
        if (!await teacher.checkPassword(password))
            throw new routing_controllers_1.BadRequestError('The password is not correct');
        const jwt = jwt_1.sign({ id: teacher.id, email: teacher.email });
        return { jwt };
    }
};
__decorate([
